#! /usr/bin/env python3
import sys #biblioteca de importação de parametro, a stream virá ao script como um parametro
import pickle #exportação do modelo de ml para um arquivo para uso posterior
import pandas as pd

Original_features =['tcp_stream','udp_stream','id','date1','relative_time','ip_src','ip_dst','service','length','protocol','ip_flag','tcp_src_port','tcp_dst_port','tcp_flag','udp_src_port','udp_dst_port','urgent','window_size']

Stream_features=['duration','src_bytes','dst_bytes','protocol_type','service','land','flags','length','window_size','urgent','counts','serror_rate','rerror_rate','same_srv_rate','diff_srv_rate','srv_count','srv_serror_rate','srv_rerror_rate','srv_diff_host_rate']
...
#     return(total_length,total_window,urgent,classe)
    return(total_length,total_window,urgent)
#==================================================================================================================
...
def import_data(path): #recebe como parâmetro o argumento passado na chamada do scripr (os pacotes da stream)
#     print('Importing network logs for analisys...')
    stream = pd.read_csv(path,names=(Original_features)) #bug remove 1 feature 18 vs 19 (date2)
    #Tratamento por protocol
    if(sys.argv[2] == 'tcp'):
        del stream['udp_stream'];
        del stream['udp_src_port']
        del stream['udp_dst_port']
        stream['src_port'] = stream['tcp_src_port']
        stream['dst_port'] = stream['tcp_dst_port']
        del stream['date1']
        del stream['tcp_src_port']
        del stream['tcp_dst_port']
#             stream['classe'] = 0
    elif(sys.argv[2] == 'udp'):
        del stream['tcp_stream'];
        del stream['tcp_src_port'];
        del stream['tcp_dst_port'];
        stream['src_port'] = stream['udp_src_port']
        stream['dst_port'] = stream['udp_dst_port']
        del stream['date1']
        del stream['udp_src_port']
        del stream['udp_dst_port']
#             stream['classe'] = 0
...
#     print('Processing network data streams...')
    if(stream.empty == True):print('Empty pkgs!') # Caso o dataframe esteja vazio, continuar sem dar error
...
def ML_preprocess(data):
    from sklearn.preprocessing import LabelEncoder
    le = LabelEncoder()
    #Literal para int
    data.protocol_type = le.fit_transform(data.protocol_type.astype(str))
    data.service = le.fit_transform(data.service.astype(str))
    data.flags = le.fit_transform(data.flags.astype(str))
    return(data)
#==================================================================================================================
...
stream = import_data(sys.argv[1]) #import single stream
