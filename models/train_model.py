...
Original_features = ['count','date1','date2','relative_time','ip_src','ip_dst','service','length','protocol',
                     'ip_flag','src_port','dst_port','tcp_flag','urgent','window_size']

Stream_features=['duration','src_bytes','dst_bytes','protocol_type','service','land',
                 'flag','length','window_size','urgent','classe','counts','serror_rate','rerror_rate',
...
def ML_preprocess(series_processed):
    ...
    series_processed.flag = le.fit_transform(series_processed.flag.astype(str))
    return(series_processed)
#==================================================================================================================
...
def train_data(series_processed):
    ...
    ml_features=['duration','protocol_type','service','flag','src_bytes','dst_bytes',
                 'land','urgent','counts','srv_count','serror_rate','srv_serror_rate','rerror_rate',
...
#     print(bcolors.FAIL + "Warning: No active frommets remain. Continue?" + bcolors.ENDC)
    trained_model = pickle.load(open('./trained_model_darpa.sav','rb')) #import model from a file
...
def train_data(series_processed):
    ...
    ml_features=['duration','src_bytes','dst_bytes','protocol_type','service','land', #Sem Classe
                 'flag','urgent','counts','serror_rate','rerror_rate',
                 'same_srv_rate','diff_srv_rate','srv_count','srv_serror_rate',
                 'srv_rerror_rate','srv_diff_host_rate']
    #___divide o dataset em 4: Xtrain = features selecionadas(sem a feature classe) Ytrain(feature classe como respotas para se aprender com as demais features), Xtest que são as features que servirão para validar o aprendizado feito em train, e Ytest que servirá como indicador de quantas vezes o algoritmo acertou e errou
    X_train, X_test, y_train, y_test = train_test_split(darpa_train[ml_features],
...
import pickle
lists_imported = train_model_darpa()
...
streams_dataframe = ML_preprocess(streams_dataframe)
Predict_train(streams_dataframe)

# pickle.dump(trained_model,open('trained_model_darpa.sav','wb')) # #Salva o modelo treinado em um arquivo que pode ser 
